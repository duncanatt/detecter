...
-include_lib("cow_inline.hrl").
-include_lib("cow_parse.hrl").
...
loop(State=#state{parent=Parent, socket=Socket, transport=Transport, opts=Opts,
	...
	receive
		...
		{OK, Socket, _} when OK =:= element(1, Messages), InStreamID > LastStreamID ->
			?TRACE("Discarding data that came in after the last request."),
			loop(State);
		%% Socket messages.
		{OK, Socket, Data} when OK =:= element(1, Messages) -> % This message comes from the gen_tcp library used in module ranch_tcp.
			{ok, _Port} = inet:port(Socket),
			?TRACE("Read ~w payload from port ~w (~w).", [OK, _Port, Socket]),
			?TRACE("Parsing data payload ~p..", [Data]),
			parse(<< Buffer/binary, Data/binary >>, State);
		{Closed, Socket} when Closed =:= element(2, Messages) -> % Sent by the gen_tcp library when the socket is closed (in module ranch_tcp)).
			?TRACE("Terminating socket connection ~p successfully.", [Socket]),
			terminate(State, {socket_error, closed, 'The socket has been closed.'});
		{Error, Socket, Reason} when Error =:= element(3, Messages) ->
			?TRACE("Terminating socket connection ~p with error ~p.", [Socket, Error]),
		...
		{system, From, Request} ->
			?TRACE("Handling system message."),
			sys:handle_system_msg(Request, From, Parent, ?MODULE, [], State);
		%% Messages pertaining to a stream.
		_X = {{Pid, StreamID}, Msg} when Pid =:= self() ->
			?TRACE("Info ~p.", [_X]),
		...
		{'$gen_call', From, Call} ->
			?TRACE("Handling supervisor call ~p from ~p.", [Call, From]),
			cowboy_children:handle_supervisor_call(Call, From, Children, ?MODULE),
			loop(State);
		%% Unknown messages.
		Msg ->
			?WARN("Received stray message ~p.", [Msg]),
			cowboy:log(warning, "Received stray message ~p.~n", [Msg], Opts),
			loop(State)
	after InactivityTimeout ->
		?TRACE("Terminating protocol handler after ~wms of inactivity.", [InactivityTimeout]),
...
%%	?TRACE("Parsing request from buffer 1."),
%%	?TRACE("Empty lines: ~p", [EmptyLines]),
...
%%	?TRACE("Parsing request from buffer 2."),
%%	?TRACE("Headers: ~p.", [Headers]),
...
%%	?TRACE("Parsing request from buffer 3."),
...
%%	?TRACE("Parsing request from buffer 4."),
...
after_parse({request, Req=#{streamid := StreamID, method := Method,
		...
		?TRACE("Extracted ~s request for ~s.", [Method, Version]),
	try cowboy_stream:init(StreamID, Req, Opts) of
		{Commands, StreamState} ->
%%			?TRACE("--- After initializing the stream and obtaining commands: ~p.", [Commands]),
...
%%			?TRACE("--- Before parsing with commands: ~p.", [Commands]),
