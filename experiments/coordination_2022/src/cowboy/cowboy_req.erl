...
reply(_, _, _, #{has_sent_resp := _}) ->
	?ERROR("Function clause in request."),
	error(function_clause); %% @todo Better error message.
reply(Status, Headers, {sendfile, _, 0, _}, Req)
		when is_integer(Status); is_binary(Status) ->
	?TRACE("Send file with content length 0."),
	do_reply(Status, Headers#{
		<<"content-length">> => <<"0">>
	}, <<>>, Req);
reply(Status, Headers, SendFile = {sendfile, _, Len, _}, Req)
		when is_integer(Status); is_binary(Status) ->
	?TRACE("Send file with content length ~p.", [Len]),
	do_reply(Status, Headers#{
...
reply(Status, Headers, Body, Req)
	...
	?TRACE("Send with status ~p.", [Status]),
	0 = iolist_size(Body),
...
reply(Status, Headers, Body, Req)
		when is_integer(Status); is_binary(Status) ->
	?TRACE("Send with status ~p and content length ~s.", [Status, integer_to_binary(iolist_size(Body))]),
	do_reply(Status, Headers#{
...
cast(Msg, #{pid := Pid, streamid := StreamID}) ->
	...
	?TRACE("Cast message to ~w on stream #~w: ~p.", [Pid, StreamID, Msg]),
	ok.
