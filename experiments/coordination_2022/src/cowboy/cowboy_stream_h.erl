...
%%	?TRACE("Connection process spawned request process ~w to handle ~p.", [Pid, Req]),
...
request_process(Req, Env, Middlewares) ->
	?TRACE("Started new request process to execute ~p middleware(s) on request ~p.", [Middlewares, Req]),
	try
...
execute(Req, Env, [Middleware|Tail]) ->
	?TRACE("Executing ~w middleware.", [Middleware]),
