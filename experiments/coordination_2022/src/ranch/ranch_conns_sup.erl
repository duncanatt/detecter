...
start_protocol(SupPid, Socket) ->
  ...
  _Msg = SupPid ! {?MODULE, start_protocol, self(), Socket}, % Sends start protocol message to the connections supervisor.
  ?TRACE("Notified ~w to start protocol on port ~w (~w), ~w.", [SupPid, _Port, Socket, _Msg]),
  receive SupPid -> ?TRACE("Received start protocol confirmation."), ok end. % Unblocks the calling acceptor.
...
loop(State = #state{parent = Parent, ref = Ref, conn_type = ConnType,
  ...
  receive
    {?MODULE, start_protocol, To, Socket} ->
      try Protocol:start_link(Ref, Socket, Transport, Opts) of % Starts the protocol handler implementation.
        {ok, Pid} ->
          {ok, _Port} = inet:port(Socket),
          ?TRACE("Started new connection process ~w (~w) on port ~w (~w) on ~w transport.", [Protocol, Pid, _Port, Socket, Transport]),
          handshake(State, CurConns, NbChildren, Sleepers, To, Socket, Pid, Pid); % Sends the unblocking message to the protocol handler.
...
handshake(State = #state{ref = Ref, transport = Transport, handshake_timeout = HandshakeTimeout,
  ...
  case Transport:controlling_process(Socket, ProtocolPid) of
    ok ->
      _Msg = ProtocolPid ! {handshake, Ref, Transport, Socket, HandshakeTimeout},
      {ok, _Port} = inet:port(Socket),
      ?TRACE("Sending handshake to connection process ~w for port ~w (~w) on ~w transport, ~w.", [ProtocolPid, _Port, Socket, Transport, _Msg]),
      put(SupPid, active),
      CurConns2 = CurConns + 1,
      if CurConns2 < MaxConns ->
        To ! self(), % Send reply back to the acceptor as a confirmation that the connection process has been created, and to unblock the acceptor To.
        ?TRACE("Sent start protocol confirmation to ~w.", [To]),
