...
listen(Opts) ->
  ...
  %% first argument.
  _Ret = {ok, _Socket} = gen_tcp:listen(0, _Opts = ranch:filter_options(Opts5, disallowed_listen_options(),
    [binary, {active, false}, {packet, raw}, {reuseaddr, true}])),
	{ok, _Port} = inet:port(_Socket),
  ?TRACE("Listening on port ~w with socket options ~p.", [_Port, _Opts]),
  _Ret.
...
send(Socket, Packet) ->
  ...
  ?TRACE("Sending data to port ~w (~w): ~p.", [_Port, Socket, Packet]),
...
controlling_process(Socket, Pid) ->
	{ok, _Port} = inet:port(Socket),
	?TRACE("~w transferred control of port ~w (~w) to ~w.", [self(), _Port, Socket, Pid]),
...
-spec shutdown(inet:socket(), read | write | read_write)
      -> ok | {error, atom()}.
shutdown(Socket, How) ->
  ?TRACE("Shutting down socket ~p (~p).", [Socket, How]),
