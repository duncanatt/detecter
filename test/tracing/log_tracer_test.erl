...
%%% @version 0.9
%%%
%%% @doc Tests the {@link log_tracer_test} module.
%%%
%%% @end
%%% Created: 14. Feb 2019 09:55
%%%
    ...
    [
      ...
      fun(_) ->
        {"Preempting an unsubscribed tracee does not succeed", ?_test(
          ...
          % Preempt tracer of process P_1 with this process as the new tracer.
      ...
      {[{delay, 0, {send, ?P1, ?P2, msg}},
        ...
        fun(Trace, _) ->
          {"A process event is dispatched to the tracer allocated for the
            ...
            % Allocate current process as the tracer of P_1. The allocation
        ...
        fun(Trace, _) ->
          {"A process fork event is dispatched to the tracer allocated for the
            ...
            % persists since P_1 does not terminate.
              ...
              % allocated to the forked processes P_2. This allocation
              ...
              % posted trace event (P_1 is already allocated a tracer prior to
        ...
        fun(Trace, _) ->
          {"A process event is dispatched to the tracer allocated for the
            ...
            % does not persist since P_1 terminates.
              ...
              % Assert that the tracer allocated to P_1 is now deallocated
        ...
        fun(Trace, _) ->
          {"Transitively related process fork events are dispatched to the
            ...
            % does not persist since P_1 terminates.
              ...
              % Assert that the tracer allocated to P_1, also automatically
              % allocated to P_2 and P_3, is now deallocated since all three
        ...
        fun({Trace1, Trace2}, _) ->
          {"Process events being dispatched to one tracer process are dispatched
            ...
            % Post first part of event trace to log tracer.
              ...
              % Retrieve result from P_1's (now terminated) tracer. Assert
              % that the event dispatched to it is indeed the correct one.
              {Result3, Event} = yield(Ref),
              ?assert(Result3),
              ?assertEqual(event:to_evm_event(unwrap_delay(lists:nth(2, Trace2))), Event),

              % Assert that the event dispatched to the tracer for P_2 is
              % indeed correct. Dispatching in this case is performed
              % incrementally with each posted trace event (P_2 is already
              ...
              % Assert that the tracer allocated to P_1 and that allocated to
              % P_2 are now deallocated since both processes terminate. This
...
%%   `fork(P_2, P_3),recv(P_2),fork(P_1, P_2),send(P_3, P_2),exit(P_2)'
...
%%   process `P_1' triggers a reprocessing of the backlog as follows:
%% }
%% {@ol
%%   {@item `fork(P_2, P_3)' is processed and kept in the backlog.}
%%   {@item `recv(P_2)' is processed and kept in the backlog.}
%%   {@item `fork(P_1, P_2)' is processed and dispatched to `T_1'; also, tracer
%%          `T_1' is automatically allocated to the newly-forked process `P_1'
%%   }
%% }
%% {@par At this point, the backlog can be processed in one of two ways. If the
%%       remaining events naively employ the new allocation of `T_1' to `P_2',
%%       then `send(P_3, P_2)' is processed and kept in the backlog, followed by
%%       `exit(P_2)' which is instantly dispatched to `T_1'. It is not hard to
%%       see that once the trace events at the head of the backlog, namely,
%%       `fork(P_2, P_3)' and `recv(P_2)', are processed, `T_1' is left with the
%%       trace `exit(P_2),fork(P_2, P_3),recv(P_2)', which is clearly not
%%       desired as the reordering of events portray an unsound execution for
%%       process `P_2'. The problem can be eliminated if the backlog is
%%       reprocessed as <i>soon as a new tracer allocation is detected</i>,
%%       resulting in the trace `fork(P_2, P_3),recv(P_2),exit(P_2)' being
          ...
          {"Process events are dispatched to the tracer in their original order
            ...
            ?_test(
              ...
              % persists since P_1 does not terminate.
                ...
                % tracer depends on its allocation. In this case, P_1 is
                % allocated a tracer that is also automatically allocated to P_2
                % and P_3. Consequently, all events in the trace are dispatched
                % to it.
        ...
        fun(Trace, _) ->
          {"Process events are dispatched to the tracer in their original order
            ...
            % Post event trace to log tracer.
              ...
              % depends on its allocation. In this case, P_1 is allocated a
              % tracer that is also automatically allocated to P_2 and P_3.
          ...
          {"Process events are dispatched to the tracer in their original order
            ...
            ?_test(
              ...
              % Post event trace to log tracer.
                ...
                % Allocate current process as the tracer of P_2. The allocation
                ...
                % tracer depends on its allocation. In this case, P_2 is
        ...
        fun(Trace, _) ->
          {"Process events are dispatched to the tracer in their original order
            ...
            % does not persist since P_1 terminates.
              ...
              % Assert that the tracer allocated to P_1, also automatically
              % allocated to P_2, P_3 and P_4, is now deallocated since all four
              % processes terminate. This deallocation is performed
              % incrementally with each posted trace event (P_1 is already
              % allocated a tracer prior to posting the trace), and waiting for
              % it complete is not necessary.
              ...
              % the trace, the order in which these are delivered to the tracer
              % depends on its allocation. In this case, P_1 is allocated a
              % tracer that is also automatically allocated to P_2, P_3 and P_4.
              % Consequently, all events in the trace are dispatched to it.
              Expected = [
                {fork, ?P1, ?P2, {m, f, []}},
                {recv, ?P2, msg},
...
%% @doc Executes the specified `Fun' asynchronously in its own process and
%% returns the result to the invoker when ready.
%%
%% {@params
%%   {@name Fun}
...
%% }
%%
%% {@returns a reference corresponding to the function execution ID.}
-spec promise(fun()) -> reference().
promise(Fun) ->
  This = self(),
  Ref = erlang:make_ref(),
  Pid = spawn(fun() -> This ! {ok, Ref}, This ! {promise, Ref, Fun()} end),
  receive
    {ok, Ref} -> {Pid, Ref}
  end.

%% @doc Retrieves the result of a {@link promise/1} identified by `Ref'.
%%
%% {@params
%%   {@name Ref}
%%   {@desc The promise reference.}
%% }
%% {@par In the promise identified by `Ref' has not finished executing, the
%%       caller is blocked.
%% }
%%
%% {@returns the promise result.}
-spec yield(reference()) -> term().
yield(Ref) ->
  receive
    {promise, Ref, Result} ->
      Result
...
%% @private Translates the log events to onees that is identical to what the EVM
%% generates.
to_evm_events([]) ->
  [];
