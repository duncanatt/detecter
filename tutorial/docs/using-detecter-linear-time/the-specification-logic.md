...
[comment]: <> (For the sake of simplicity, the script `hello_prop.hml` used in our hello world example specifies a sHML property that every system necessarily violates, namely, the formula `#!shml ff` that denotes falsity.)

[comment]: <> (We now look at the full sHML grammar that enables us to express more interesting and useful properties *about programs* &#40;not traces&#41;.)

[comment]: <> (Specifications in sHML---also called *formulae*---are interpreted over the *states* of transition models of programs we wish to analyse.)

[comment]: <> (sHML is a syntactic subset of the more expressive Hennessy-Milner Logic with recursion, and is used to specify [safety properties]&#40;getting-started.md#safety-properties&#41;.)

[comment]: <> (Recall safety properties state that "something bad should never happen".)

[comment]: <> (sHML formulae are generated from the following grammar:)

[comment]: <> (By contrast to the logic fragment sHML covered in [formalising-properties], )

maxHML enables us to express properties on the traces of programs.
Like sHML, maxHML is a another syntactic subset of the more expressive Hennesy-Milner Logic with recursion: it is also used to express safety properties, but by contrast to the sHML fragment of [formalising-properties], is interpreted over *traces*. 
...
7. `#!maxhml max(X. φ)` is the *maximal fix-point* construct that specifies *recursion* via the logical variable `#!maxhml X` and *binds* the free occurrences of `#!maxhml X` in the sub-formula `#!maxhml φ`. 
...
Let us try to specify a safety requirement on the *traces* of our [calculator program](../using-detecter/getting-started.md#calculator-program).
...
This means that the `#!maxhml [Srv:Clt ! {bye, Tot} when Tot < 0]ff` can only be satisfied when our trace does not exhibit the event described by the symbolic action `#!erlang Srv:Clt ! {bye, Tot} when Tot < 0`.
...
You should try expressing the safety properties P~1~, P~2~, P~3~, and P~4~ stated informally in the [Getting Started](../using-detecter/getting-started.md) section in terms of maxHML.
